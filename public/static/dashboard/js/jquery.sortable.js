 ...
 * http://farhadi.ir/projects/html5jq_sortable
 * 
...
(function ($) {
  ...
  $.fn.jq_sortable = function (options) {
    ...
    return this.each(function () {
      ...
      var placeholder = $('<' + (/^ul|ol$/i.test(this.tagName) ? 'li' : /^tbody$/i.test(this.tagName) ? 'tr' : 'div') +
                          ' class="jq_sortable-placeholder ' + options.placeholderClass + '">').html('&nbsp;');
      ...
      items.attr('draggable', 'true').on('dragstart.h5s', function (e) {
        ...
        index = (dragging = $(this)).addClass('jq_sortable-dragging').index();
        parent = dragging.parent();
      }).on('dragend.h5s', function () {
        if (!dragging) {
          return;
        }
        dragging.removeClass('jq_sortable-dragging').show();
        placeholders.detach();
